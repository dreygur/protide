...
impl SyncEngine {
    ...
    pub fn update_local_change(
        &mut self,
        id: uuid::Uuid,
        data_type: DataType,
        data: String,
    ) -> Option<types::CrdtEntry> {
            ...
            for p2p_event in p2p_events {
                match p2p_event {
                    ...
                    p2p::P2PEvent::PeerJoined(peer) => {
                        ...
                        events.push(SyncEvent::P2PDiagnostic(format!(
                    ...
                    p2p::P2PEvent::PakeMsg {
                        ...
                        info!(
                            "[PAKE] Received '{}' from {} on topic {}",
                            kind, from, topic
                        );
                        events.push(SyncEvent::P2PDiagnostic(format!(
                            "[PAKE] Received '{}' from {} on {}",
                            kind, from, topic
                        )));
    ...
    pub fn initiate_handshake(&mut self, _code: &str) -> Result<(), String> {
        #[cfg(all(feature = "p2p-sync", feature = "pake-auth"))]
        {
            ...
            if let Some(ref p2p) = self.p2p_sync {
                ...
                info!(
                    "[PAKE] Init packet published on a hashed PAKE topic (code not sent over the wire)"
                );
