...
/// Creates a fresh runtime for each execution to avoid GC issues
...
impl JsRuntime {
    ...
    pub fn execute(
        ...
        // Create fresh runtime and context for each execution
        ...
        let result = context.with(|js_ctx| {
            // Set up the scripting environment with global storage
            ...
            js_ctx
                .eval::<Value, _>(script)
                .map_err(|e| ScriptError::new(format!("Script error: {}", e)))?;

            // Extract results from JS storage
            extract_results(&js_ctx, ctx)
        });

        // Run GC before dropping
        runtime.run_gc();
