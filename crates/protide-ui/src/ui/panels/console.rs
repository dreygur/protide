...
use gpui::{
    div, prelude::*, px, ClipboardItem, Context, IntoElement, MouseButton,
    ParentElement, Render, ScrollHandle, SharedString, Styled, Window,
};

use crate::theme;
use crate::ui::components::icons::{icon, ICON_SM, ICON_CLOSE, ICON_COPY, ICON_ELLIPSIS};
            ...
            .child(
                div()
                    ...
                    .children(entries.iter().enumerate().map(|(i, entry)| {
                        ...
                        let row_color = match entry.level {
                        ...
                        let level_bar = match entry.level {
                            LogLevel::Error => theme.colors.error,
                            LogLevel::Debug => theme.colors.info,
                            LogLevel::Info  => theme.colors.success.opacity(0.0), // invisible
                        };
                        ...
                        div()
                            ...
                            // Left severity bar (3px, Debug=blue, Error=red, Info=invisible)
                            .child(
                                div()
                                    .w(px(3.0))
                                    .h(px(24.0))
                            ...
                            // Level badge (DEBUG only — INFO/ERROR are implicit via color)
...
fn level_chip(label: &'static str, color: gpui::Hsla, theme: &crate::theme::Theme) -> gpui::AnyElement {
    div()
        .px(px(5.0))
        .py(px(1.0))
