...
impl ExplorerPanel {
    pub fn import_collection(&mut self, cx: &mut Context<Self>) {
        file_dialog::prompt(
            ...
            |d| {
                ...
                match last_paths::last_dir("import_collection") {
                ...
                if let Ok(content) = fs::read_to_string(&file_path) {
                    match protide_core::import::import(&content) {
                        Ok(result) => {
                            ...
                            if this.workspace_path.is_some() {
                                this.collection_items = this.scan_directory(&output_dir);
                            } else {
                                this.load_collection_from_path(collection_dir, cx);
                            }
    ...
    pub(super) fn export_openapi_folder(&mut self, path: PathBuf, cx: &mut Context<Self>) {
        ...
        file_dialog::prompt(
            ...
            Pick::Save,
            |d| {
                d.set_title("Export OpenAPI 3.0")
    ...
    pub fn export_docs(&mut self, cx: &mut Context<Self>) {
        ...
        file_dialog::prompt(
            ...
            Pick::Save,
            |d| {
                d.set_title("Export API Documentation")
