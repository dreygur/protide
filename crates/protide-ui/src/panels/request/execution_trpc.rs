...
impl<E: WebSocketExecutor> RequestPanel<E> {
    pub(super) fn send_trpc_request(&mut self, cx: &mut Context<Self>) {
        ...
        cx.notify();
        self.response_panel
            .update(cx, |panel, cx| panel.set_loading(cx));
    ...
    pub(super) fn run_trpc_playground(&mut self, cx: &mut Context<Self>) {
        let idx = match self.trpc_pg_selected {
            Some(i) => i,
            None => return,
        };
        let proc = match self.trpc_pg_procedures.get(idx) {
            Some(p) => p.clone(),
            None => return,
        };
        ...
        file_dialog::prompt(
            ...
            |this, path, cx| {
                    ...
                    .and_then(|s| protide_core::protocols::trpc::parse_trpc_schema(&s))
                {
                    Ok(procs) => {
                        for p in procs {
                            let proc = TrpcPlaygroundProc {
                                kind: if p.is_mutation {
                                    TrpcProcKind::Mutation
                                } else {
                                    TrpcProcKind::Query
                                },
                                name: p.name,
                            ...
                            match this
                                .trpc_pg_procedures
                                .iter_mut()
                                .find(|existing| existing.name == proc.name)
                            {
                    ...
                    Err(e) => {
                        this.trpc_pg_schema_error = Some(e);
                    }
                }
                cx.notify();
            },
                    ...
                    let _ = this.update(cx, |panel, cx| {
                        ...
                        match result {
                            Ok(procs) => {
                                ...
                                panel.queue_editor(
                                    PendingEditor::TrpcPgImportUrlInput,
                                    String::new(),
                                );
                            }
                            Err(e) => {
                                panel.trpc_pg_schema_error = Some(e);
                            }
                        }
                        cx.notify();
                    });
    ...
    pub(super) fn fetch_trpc_schema(&mut self, cx: &mut Context<Self>) {
        ...
        let env_state = self
            ...
            .as_ref()
            .map(|p| p.read(cx).env_state().clone());
        let url = if let Some(ref env) = env_state {
            env.substitute(&url)
        } else {
            url
        };
            ...
            async move |this: gpui::WeakEntity<Self>, cx: &mut gpui::AsyncApp| {
                ...
                let _ = cx.update(|cx| {
                    let _ = this.update(cx, |panel, cx| {
                        ...
                        match result {
                            Ok(procs) => {
                                panel.trpc_pg_procedures = procs
                                    .into_iter()
                                    .map(|p| TrpcPlaygroundProc {
                                        kind: if p.is_mutation {
                                            TrpcProcKind::Mutation
                                        } else {
                                            TrpcProcKind::Query
