...
impl<E: WebSocketExecutor> RequestPanel<E> {
    pub(super) fn get_edit_text(&self, target: EditTarget) -> &str {
        match target {
            ...
            EditTarget::HeaderValue(i) => {
                self.headers.get(i).map(|h| h.value.as_str()).unwrap_or("")
            }
            ...
            EditTarget::GrpcMetaKey(i) => self
                .grpc_metadata
                .get(i)
                .map(|m| m.key.as_str())
                .unwrap_or(""),
            EditTarget::GrpcMetaValue(i) => self
                .grpc_metadata
                .get(i)
    ...
    pub(super) fn edit_cursor(&self) -> usize {
        self.edit_selection.end
    }
    pub(super) fn edit_has_selection(&self) -> bool {
    ...
    pub(super) fn edit_index_for_x(&self, x: f32, char_width: f32) -> usize {
        if let Some(target) = self.active_edit {
            ...
            if x <= 0.0 {
                ...
                ((x / char_width) as usize).min(char_count)
            }
        } else {
            0
        }
    ...
    pub(super) fn handle_edit_mouse_move(
        ...
        if self.edit_is_selecting {
            let text_start_x = self
                ...
                .unwrap_or(0.0);
            let index = self.edit_index_for_x(
                (f32::from(event.position.x) - text_start_x).max(0.0),
                char_width,
            );
