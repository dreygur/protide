...
use crate::components::icons::{
    ICON_CHECK, ICON_CHEVRON_DOWN, ICON_EDIT, ICON_PLUS, ICON_SM, icon,
};
use crate::theme;
...
impl<E: WebSocketExecutor> RequestPanel<E> {
    pub(super) fn render_pg_proc_list(&mut self, cx: &mut Context<Self>) -> gpui::AnyElement {
        ...
        for &idx in &filtered {
            ...
            let key = if r.is_empty() {
                "(root)".to_string()
            } else {
                r.to_string()
            };
            if !routers.contains(&key) {
                routers.push(key);
            }
        }
        ...
        div()
            ...
            .children(routers.into_iter().map(|router| {
                ...
                let is_editing_group =
                    self.trpc_pg_editing_group.as_deref() == Some(router.as_str());
                    ...
                    div()
                        ...
                        .child(
                            div().flex_1().h_full().overflow_hidden().child(
                                Input::new(&self.trpc_pg_group_edit_input)
                                    .bordered(false)
                            ...
                            div()
                                ...
                                .child(
                                    div()
                                        ...
                                        .items_center()
                                        .justify_center()
                                        .bg(if edit_kind == TrpcProcKind::Query {
                                        ...
                                        .text_color(if edit_kind == TrpcProcKind::Query {
                                        ...
                                        .on_click(cx.listener(|this, _, _, cx| {
                                            this.trpc_pg_edit_kind = TrpcProcKind::Query;
                                            cx.notify();
                                        }))
                                        .child("Q"),
                                )
                                ...
                                .child(
                                    div()
                                        ...
                                        .items_center()
                                        .justify_center()
                                        .bg(if edit_kind == TrpcProcKind::Mutation {
                                            theme.colors.method_post.opacity(0.12)
                                        } else {
                                            theme.colors.bg_secondary
                                        })
                                        .text_size(px(9.0))
                                        .font_weight(gpui::FontWeight::EXTRA_BOLD)
                                        .text_color(if edit_kind == TrpcProcKind::Mutation {
                                        ...
                                        .cursor_pointer()
                                        .on_click(cx.listener(|this, _, _, cx| {
                                            this.trpc_pg_edit_kind = TrpcProcKind::Mutation;
                                            cx.notify();
                                        }))
                                        .child("M"),
                                )
                                ...
                                .child(
                                    div().flex_1().h_full().overflow_hidden().child(
                                        Input::new(&self.trpc_pg_edit_input)
                                            .bordered(false)
                                            .with_size(gpui_component::Size::XSmall),
                                    ),
                                )
                            ...
                            div()
                                ...
                                .child(
                                    div()
                                        ...
                                        .text_color(kc)
                                        .child(match kind {
                                            TrpcProcKind::Query => "Q",
                                            TrpcProcKind::Mutation => "M",
                                        }),
                                )
                                // Leaf name
                                .child(
                                        ...
                                        .on_mouse_down(
                                            ...
                                            cx.listener(move |this, _, _, cx| {
                                                ...
                                                match this.trpc_pg_selected {
                                                    Some(s) if s == idx => {
                                                        this.trpc_pg_selected = None
                                                    }
                                                    Some(s) if s > idx => {
                                                        this.trpc_pg_selected = Some(s - 1)
                                                    }
                                                    _ => {}
                                                }
                                                cx.notify();
        ...
        div()
            ...
            .child(
                chip(
                    "trpc-pg-kind-q",
                    "Q",
                    add_kind == TrpcProcKind::Query,
                    theme.colors.method_get,
                    theme.colors.method_get.opacity(0.12),
                    theme.colors.method_get,
                )
                .on_click(cx.listener(|this, _, _, cx| {
                    this.trpc_pg_add_kind = TrpcProcKind::Query;
                    cx.notify();
                })),
            )
            .child(
                chip(
                    "trpc-pg-kind-m",
                    "M",
                    add_kind == TrpcProcKind::Mutation,
                    theme.colors.method_post,
                    theme.colors.method_post.opacity(0.12),
                    theme.colors.method_post,
                )
                .on_click(cx.listener(|this, _, _, cx| {
                    this.trpc_pg_add_kind = TrpcProcKind::Mutation;
                    cx.notify();
                })),
            )
            .child(
                div()
                    ...
                    .overflow_hidden()
                    .child(
                        Input::new(&self.trpc_pg_add_input)
                            .bordered(false)
                            .with_size(gpui_component::Size::XSmall),
                    ),
            )
                ...
                div()
                    ...
                    .on_click(cx.listener(|this, _, _, cx| {
                        ...
                        if !name.is_empty() {
                            ...
                            this.trpc_pg_procedures
                                .push(TrpcPlaygroundProc { kind, name });
