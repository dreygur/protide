...
fn test_http_method_from_str() {
    ...
    assert_eq!(
        HttpMethod::from_str("INVALID"),
        Some(HttpMethod::Custom("INVALID".to_string()))
    );
...
fn test_url_encode_decode_roundtrip() {
    let test_cases = vec!["simple", "with spaces", "special=chars&here", "numbers123"];
...
fn test_char_count_not_byte_len_for_next_field_cursor() {
    ...
    assert_ne!(
        char_len, byte_len,
        "test text must contain a multi-byte character"
    );
