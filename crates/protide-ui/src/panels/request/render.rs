...
use gpui::{
    Context, IntoElement, KeyDownEvent, MouseButton, MouseMoveEvent, ParentElement, Render, Styled,
        ...
        div()
            ...
            .capture_key_down(cx.listener(|this, event: &KeyDownEvent, _, cx| {
                ...
                if event.keystroke.key == "escape" {
                    ...
                    if this.grpc_service_picker_open {
                        this.grpc_service_picker_open = false;
                        cx.notify();
                        return;
                    }
                    if this.grpc_method_picker_open {
                        this.grpc_method_picker_open = false;
                        cx.notify();
                        return;
                    }
                }
            ...
            .child({
                ...
                if is_pg {
                    ...
                    base.p(px(12.0)).overflow_scroll()
                }
            ...
            .when(self.kv_row_drag.is_some(), |el| {
                el.child(
                    deferred(
                        div()
                            .id("kv-row-drag-overlay")
                            .absolute()
                            .top_0()
                            .left_0()
                            .w_full()
            ...
            .when(self.form_row_drag.is_some(), |el| {
                el.child(
                    deferred(
                        div()
                            .id("form-row-drag-overlay")
                            .absolute()
                            .top_0()
                            .left_0()
                            .w_full()
