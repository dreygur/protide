...
mod tests {
    ...
    async fn test_copy_json_selection_multibyte_utf8_no_panic(cx: &mut TestAppContext) {
        ...
        let end_offset = panel.update(cx, |p, _cx| {
            use crate::panels::response::{GUTTER_W, INDENT_W, CHEVRON_W, JSON_CHAR_W};
            let bounds = p.json_tree_bounds.unwrap_or_default();
            let row = &p.json_rows[1];
            let key_chars = 1 + 4; // "a":
            let val_x = f32::from(bounds.origin.x) + GUTTER_W + (row.depth as f32) * INDENT_W
                + CHEVRON_W + (key_chars as f32) * JSON_CHAR_W;
        ...
        assert_eq!(end_offset, 4, "expected byte offset just past the quote + '日' (1 + 3 bytes)");
        ...
        panel.read_with(cx, |p, cx| {
            ...
            assert_eq!(clip.as_deref(), Some("\"日"), "expected the selection to cover the opening quote + '日'");
        });
    }
}
