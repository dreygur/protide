...
pub(super) fn render_json_row(
    ...
    let guides: Vec<gpui::AnyElement> = (0..depth)
        .map(|_| {
            let g = div()
                .w(px(INDENT_W))
                .flex_shrink_0()
            ...
            g.child(div().w(px(1.0)).h_full().flex_shrink_0().bg(guide_color))
                .into_any_element()
        })
        .collect();
    ...
    let chevron = {
        ...
        c.when(is_open, |el| {
            el.child(icon(ICON_CHEVRON_DOWN, ICON_SM, colors.text_secondary))
        })
        .when(is_folded, |el| {
            el.child(icon(ICON_CHEVRON_RIGHT, ICON_SM, colors.text_secondary))
        })
    };
    ...
    let content: gpui::AnyElement = match &row.kind {
        RowKind::Leaf { key, val, .. } => {
            ...
            let (txt, col, is_str): (SharedString, _, bool) = match val {
                ...
                PrimVal::Bool(b) => (
                    SharedString::new_static(if *b { "true" } else { "false" }),
                    c_patch,
                    false,
                ),
            ...
            if wrap_mode && is_str && txt.len() > COLLAPSE_CHARS + 3 {
                ...
                let btn_label: &'static str = if expanded {
                    "▲ collapse"
                } else {
                    "▼ show more"
                };
    ...
    let ctx_items: Vec<(String, String)> = match &row.kind {
        RowKind::Leaf { key, val, path } => {
            ...
            if let Some(k) = key {
                items.push(("Copy Key".to_string(), k.clone()));
            }
            ...
            move |event: &gpui::MouseDownEvent, _, cx| {
                ...
                weak_cm
                    .update(cx, |this, cx| {
                        ...
                        this.json_context_menu = Some(JsonCtxMenu {
                            x: lx,
                            y: ly,
                            items: items_c,
                        });
